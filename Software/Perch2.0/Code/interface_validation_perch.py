...
# ─────────────────────────── CONFIG ───────────────────────────
CSV_FILE = "../results/rapport_anomalies_cosinus.csv"
AUDIO_FOLDER = "../data/SAMPLES_EXTRACTED"
...
class PerchExplorer:
    ...
    def _load_data(self):
        ...
        self.col_score = (
            "Distance_Noise" if "Distance_Noise" in self.df.columns else "Distance"
        )
    ...
    def _build_right_panel(self, parent):
        ...
        self.playback_bar = ttk.Progressbar(
            pb_frame,
            orient="horizontal",
            mode="determinate",
    ...
    def _populate_listbox(self, indices=None):
        ...
        rows = (
            self.df.iterrows()
            if indices is None
            else ((i, self.df.iloc[i]) for i in indices)
        )
        for i, row in rows:
            status = row["Human_validation"]
            badge = (
                "ANO" if (status == "Anomaly" or status == "DETECTION") else status[:5]
            )
            score = row[self.col_score]
            txt = f" [{badge:^7}]  {score:>7.4f}  │  {row['Source Audio']}"
            self.listbox.insert(tk.END, txt)
            self.listbox.itemconfig(
                tk.END, fg=STATUS_COLOR.get(status, COLORS["pending"])
            )
    ...
    def _on_search(self, *_):
        ...
        if not q:
            ...
            return
        matched = [
            i for i, row in self.df.iterrows() if q in row["Source Audio"].lower()
        ]
    ...
    def _select_row(self, idx: int):
        ...
        self.lbl_status_badge.config(
            text=status, bg=STATUS_COLOR.get(status, COLORS["pending"])
        )

        # Update comment field without triggering the auto-save trace
        self.comment_var.trace_remove(
            "write",
            (
                self.comment_var.trace_info()[0][1]
                if self.comment_var.trace_info()
                else ""
            ),
        )
    ...
    def _update_spectrogram(self, audio_path: str):
        ...
        try:
            ...
            self.ax.set_title(
                "Mel-Spectrogram", color=COLORS["text_lo"], fontsize=9, pad=6
            )
    ...
    def play_audio(self):
        ...
        path = os.path.join(
            AUDIO_FOLDER, self.df.iloc[self.current_idx]["Source Audio"]
        )
    ...
    def vote(self, choice: str):
        ...
        self.lbl_status_badge.config(
            text=choice, bg=STATUS_COLOR.get(choice, COLORS["pending"])
        )
